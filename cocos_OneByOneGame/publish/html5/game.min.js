...
k.split("-")[0]:a.LANGUAGE_ENGLISH;a.language=k;var m=!1,k=!1,u="",A=0,w=/android (\d+(?:\.\d+)+)/i.exec(h)||/android (\d+(?:\.\d+)+)/i.exec(c.platform);w&&(m=!0,u=w[1]||"",A=parseInt(u)||0);(w=/(iPad|iPhone|iPod).*OS ((\d+_?){2,3})/i.exec(h))?(k=!0,u=w[2]||"",A=parseInt(u)||0):/(iPhone|iPad|iPod)/.exec(c.platform)&&(k=!0,u="",A=0);w=a.OS_UNKNOWN;-1!==c.appVersion.indexOf("Win")?w=a.OS_WINDOWS:k?w=a.OS_IOS:-1!==c.appVersion.indexOf("Mac")?w=a.OS_OSX:-1!==c.appVersion.indexOf("X11")&&-1===c.appVersion.indexOf("Linux")?
w=a.OS_UNIX:m?w=a.OS_ANDROID:-1!==c.appVersion.indexOf("Linux")&&(w=a.OS_LINUX);a.os=w;a.osVersion=u;a.osMainVersion=A;a.browserType=a.BROWSER_TYPE_UNKNOWN;(function(){var b=/qqbrowser|qq|chrome|safari|firefox|opr|oupeng|opera/i,c=/micromessenger|mqqbrowser|sogou|qzone|liebao|ucbrowser|360 aphone|360browser|baiduboxapp|baidubrowser|maxthon|mxbrowser|trident|miuibrowser/i.exec(h);c||(c=b.exec(h));b=c?c[0]:a.BROWSER_TYPE_UNKNOWN;"micromessenger"===b?b=a.BROWSER_TYPE_WECHAT:"safari"===b&&m?b=a.BROWSER_TYPE_ANDROID:
...
var c=b.getContext("2d");c.fillStyle="#fff";c.fillRect(0,0,1,1);a.drawImage(b,0,0,1,1);return 0===a.getImageData(0,0,1,1).data[0]}();cc.sys.isMobile&&(k=document.createElement("style"),k.type="text/css",document.body.appendChild(k),k.textContent="body,canvas,div{ -moz-user-select: none;-webkit-user-select: none;-ms-user-select: none;-khtml-user-select: none;-webkit-tap-highlight-color:rgba(0,0,0,0);}");try{var z=a.localStorage=b.localStorage;z.setItem("storage","");z.removeItem("storage");z=null}catch(C){z=
function(){cc.warn("Warning: localStorage isn't enabled. Please confirm browser cookie or privacy option")},a.localStorage={getItem:z,setItem:z,removeItem:z,clear:z}}z=!!f.getContext("2d");k=!1;if(b.WebGLRenderingContext){u=document.createElement("CANVAS");try{if(cc.create3DContext(u)&&(k=!0),k&&a.os===a.OS_IOS&&9===a.osMainVersion&&(window.indexedDB||(k=!1)),k&&a.os===a.OS_ANDROID){var B=parseFloat(a.browserVersion);switch(a.browserType){case a.BROWSER_TYPE_MOBILE_QQ:case a.BROWSER_TYPE_BAIDU:case a.BROWSER_TYPE_BAIDU_APP:k=
6.2<=B?!0:!1;break;case a.BROWSER_TYPE_CHROME:k=30<=B?!0:!1;break;case a.BROWSER_TYPE_ANDROID:a.osMainVersion&&5<=a.osMainVersion&&(k=!0);break;case a.BROWSER_TYPE_UNKNOWN:case a.BROWSER_TYPE_360:case a.BROWSER_TYPE_MIUI:case a.BROWSER_TYPE_UC:k=!1}}}catch(D){}u=null}B=a.capabilities={canvas:z,opengl:k};if(void 0!==e.ontouchstart||void 0!==d.ontouchstart||c.msPointerEnabled)B.touches=!0;void 0!==e.onmouseup&&(B.mouse=!0);void 0!==e.onkeyup&&(B.keyboard=!0);if(b.DeviceMotionEvent||b.DeviceOrientationEvent)B.accelerometer=
...
(t=this._hashForTimers[b.__instanceId])?cc.assert(t.paused===h,""):(t=e.get(null,b,0,null,null,h),this._arrayForTimers.push(t),this._hashForTimers[b.__instanceId]=t);var x,y;if(null==t.timers)t.timers=[];else for(y=0;y<t.timers.length;y++)if(x=t.timers[y],a===x._callback){cc.log(cc._LogInfos.Scheduler_scheduleCallbackForTarget,x.getInterval().toFixed(4),c.toFixed(4));x._interval=c;return}x=g.get();x.initWithCallback(this,a,b,c,d,f,v);t.timers.push(x)},scheduleUpdate:function(a,b,c){this._schedulePerFrame(function(b){a.update(b)},
...
[m,k,h],u=0;3>u;++u){var A=y[u].style;A.color="rgb(0, 255, 255)";A.font="bold 12px Helvetica, Arial";A.lineHeight="20px";A.width="100%";n.appendChild(y[u])}var w=function(){e=cc.director.getSecondsPerFrame();c++;f+=cc.director.getDeltaTime();if(f>cc.DIRECTOR_FPS_INTERVAL){d=c/f;f=c=0;if(z.onFrameRateChange){var b=d,g=r.length-1,n=g,u=0;t++;for(x+=b;0<=n;n--)if(b>=r[n]){s[n]++;break}if(t>=p){u=x/p;for(n=g;0<n;n--)if(b=s[n]/p,0.6<=b&&u>=r[n]){n!=v&&(v=n,z.onFrameRateChange&&z.onFrameRateChange(u.toFixed(2)));
...
k;a[6]=c;a[7]=g;return this};b.determinant=function(){var a=this.mat,b=a[0]*a[4]*a[8]+a[1]*a[5]*a[6]+a[2]*a[3]*a[7];return b-=a[2]*a[4]*a[6]+a[0]*a[5]*a[7]+a[1]*a[3]*a[8]};b.multiply=function(a){var b=this.mat,c=a.mat;a=b[0];var g=b[1],h=b[2],k=b[3],m=b[4],n=b[5],p=b[6],r=b[7],s=b[8],v=c[0],t=c[1],x=c[2],y=c[3],u=c[4],A=c[5],w=c[6],z=c[7],c=c[8];b[0]=a*v+k*t+p*x;b[1]=g*v+m*t+r*x;b[2]=h*v+n*t+s*x;b[3]=h*v+n*t+s*x;b[4]=g*y+m*u+r*A;b[5]=h*y+n*u+s*A;b[6]=a*w+k*z+p*c;b[7]=g*w+m*z+r*c;b[8]=h*w+n*z+s*c;
...
a[2]=n;a[3]=s;a[4]=b;a[6]=p;a[7]=v;a[8]=c;a[9]=k;a[11]=t;a[12]=d;a[13]=m;a[14]=r;return this};a.kmMat4Multiply=function(a,b,c){var d=a.mat,k=b.mat,m=c.mat;c=k[0];b=k[1];var n=k[2],p=k[3],r=k[4],s=k[5],v=k[6],t=k[7],x=k[8],y=k[9],u=k[10],A=k[11],w=k[12],z=k[13],C=k[14],k=k[15],B=m[0],D=m[1],E=m[2],F=m[3],G=m[4],H=m[5],I=m[6],J=m[7],K=m[8],L=m[9],M=m[10],N=m[11],O=m[12],P=m[13],Q=m[14],m=m[15];d[0]=B*c+D*r+E*x+F*w;d[1]=B*b+D*s+E*y+F*z;d[2]=B*n+D*v+E*u+F*C;d[3]=B*p+D*t+E*A+F*k;d[4]=G*c+H*r+I*x+J*w;d[5]=
G*b+H*s+I*y+J*z;d[6]=G*n+H*v+I*u+J*C;d[7]=G*p+H*t+I*A+J*k;d[8]=K*c+L*r+M*x+N*w;d[9]=K*b+L*s+M*y+N*z;d[10]=K*n+L*v+M*u+N*C;d[11]=K*p+L*t+M*A+N*k;d[12]=O*c+P*r+Q*x+m*w;d[13]=O*b+P*s+Q*y+m*z;d[14]=O*n+P*v+Q*u+m*C;d[15]=O*p+P*t+Q*A+m*k;return a};b.multiply=function(a){var b=this.mat,c=a.mat;a=b[0];var d=b[1],k=b[2],m=b[3],n=b[4],p=b[5],r=b[6],s=b[7],v=b[8],t=b[9],x=b[10],y=b[11],u=b[12],A=b[13],w=b[14],z=b[15],C=c[0],B=c[1],D=c[2],E=c[3],F=c[4],G=c[5],H=c[6],I=c[7],J=c[8],K=c[9],L=c[10],M=c[11],N=c[12],
O=c[13],P=c[14],c=c[15];b[0]=C*a+B*n+D*v+E*u;b[1]=C*d+B*p+D*t+E*A;b[2]=C*k+B*r+D*x+E*w;b[3]=C*m+B*s+D*y+E*z;b[4]=F*a+G*n+H*v+I*u;b[5]=F*d+G*p+H*t+I*A;b[6]=F*k+G*r+H*x+I*w;b[7]=F*m+G*s+H*y+I*z;b[8]=J*a+K*n+L*v+M*u;b[9]=J*d+K*p+L*t+M*A;b[10]=J*k+K*r+L*x+M*w;b[11]=J*m+K*s+L*y+M*z;b[12]=N*a+O*n+P*v+c*u;b[13]=N*d+O*p+P*t+c*A;b[14]=N*k+O*r+P*x+c*w;b[15]=N*m+O*s+P*y+c*z;return this};a.getMat4MultiplyValue=function(a,b){var c=a.mat,d=b.mat,k=new Float32Array(16);k[0]=c[0]*d[0]+c[4]*d[1]+c[8]*d[2]+c[12]*d[3];
...
t.rect.y,t.rect.width,t.rect.height),x=cc.rectPixelsToPoints(x);x.x+=this._imageOffset.x;x.y+=this._imageOffset.y;var y=!1;if(this._spriteFrame){var u=this._spriteFrame._rect;if(this._spriteFrame._rotated){var y=!0,A=x.x;x.x=x.y+u.x;x.y=A+u.y}else x.x+=u.x,x.y+=u.y}(u=this.getChildByTag(k))?c._updateCharTexture(u,x,p,y):(u=new cc.Sprite,u.initWithTexture(d,x,y),u._newTextureWhenChangeColor=!0,this.addChild(u,0,k));u.opacityModifyRGB=this._opacityModifyRGB;c._updateCharColorAndOpacity(u);x=cc.p(e+
...
t=v%a._itemsPerRow,x=0|v/a._itemsPerRow;if(!(0>t||0>x||t*n+n>g||x*p+p>f)){s++;var y;cc.FIX_ARTIFACTS_BY_STRECHING_TEXEL?(t=(2*t*h+1)/(2*g),v=t+(2*h-2)/(2*g),x=(2*x*k+1)/(2*f),y=x+(2*k-2)/(2*f)):(t=t*h/g,v=t+h/g,x=x*k/f,y=x+k/f);var u=m[r],A=u.tl,w=u.tr,z=u.bl,u=u.br;A.texCoords.u=t;A.texCoords.v=x;w.texCoords.u=v;w.texCoords.v=x;z.texCoords.u=t;z.texCoords.v=y;u.texCoords.u=v;u.texCoords.v=y;z.vertices.x=s*n;z.vertices.y=0;z.vertices.z=0;u.vertices.x=s*n+n;u.vertices.y=0;u.vertices.z=0;A.vertices.x=
...
b.verts=a;b.lineWidth=d;b.lineColor=e;b.isClosePolygon=!0;b.isStroke=!0;b.lineCap="butt";if(b.fillColor=c)null==c.a&&(c.a=255),b.isFill=!0;this._buffer.push(b)},drawCircle:function(a,b,c,d,e,f,g){f=f||this._lineWidth;g=g||this.getDrawColor();null==g.a&&(g.a=255);for(var h=2*Math.PI/d,k=[],m=0;m<=d;m++){var w=m*h,z=b*Math.cos(w+c)+a.x,w=b*Math.sin(w+c)+a.y;k.push(cc.p(z,w))}e&&k.push(cc.p(a.x,a.y));a=new cc._DrawNodeElement(cc.DrawNode.TYPE_POLY);a.verts=k;a.lineWidth=f;a.lineColor=g;a.isClosePolygon=
...
g=g||this.getDrawColor();null==g.a&&(g.a=255);for(var h=[],k=0,m=0;m<e;m++){var w=Math.pow(1-k,3)*a.x+3*Math.pow(1-k,2)*k*b.x+3*(1-k)*k*k*c.x+k*k*k*d.x,z=Math.pow(1-k,3)*a.y+3*Math.pow(1-k,2)*k*b.y+3*(1-k)*k*k*c.y+k*k*k*d.y;h.push(cc.p(w,z));k+=1/e}h.push(cc.p(d.x,d.y));a=new cc._DrawNodeElement(cc.DrawNode.TYPE_POLY);a.verts=h;a.lineWidth=f;a.lineColor=g;a.isStroke=!0;a.lineCap="round";this._buffer.push(a)},drawCatmullRom:function(a,b,c,d){this.drawCardinalSpline(a,0.5,b,c,d)},drawCardinalSpline:function(a,
...
e,c,-1,-1),this.appendVertexData(f,a,c,1,1),this.appendVertexData(f,e,c,1,-1),this._dirty=!0)},drawSegment:function(a,b,c,d){d=d||this.getDrawColor();null==d.a&&(d.a=255);c=c||0.5*this._lineWidth;if(this._ensureCapacity(this._vertexCount+18)){f.x=a.y-b.y;f.y=b.x-a.x;cc.pNormalizeIn(f);g.x=-f.y;g.y=f.x;var e=c;h.x=f.x*e;h.y=f.y*e;k.x=g.x*c;k.y=g.y*c;c=b.x+h.x-k.x;var e=b.y+h.y-k.y,m=f.x-g.x,x=f.y-g.y,y=b.x-h.x,u=b.y-h.y,A=-f.x,w=-f.y,z=b.x+h.x,C=b.y+h.y,B=f.x,D=f.y,E=a.x-h.x,F=a.y-h.y,G=a.x+h.x,H=
a.y+h.y,I=f.x,J=f.y,K=a.x-h.x+k.x,L=a.y-h.y+k.y,M=g.x-f.x,N=g.y-f.y,O=a.x+h.x+k.x;a=a.y+h.y+k.y;var P=f.x+g.x,Q=f.y+g.y;this.appendVertexData(b.x-h.x-k.x,b.y-h.y-k.y,d,-(f.x+g.x),-(f.y+g.y));this.appendVertexData(c,e,d,m,x);this.appendVertexData(y,u,d,A,w);this.appendVertexData(z,C,d,B,D);this.appendVertexData(c,e,d,m,x);this.appendVertexData(y,u,d,A,w);this.appendVertexData(z,C,d,B,D);this.appendVertexData(E,F,d,A,w);this.appendVertexData(y,u,d,A,w);this.appendVertexData(z,C,d,B,D);this.appendVertexData(E,
F,d,A,w);this.appendVertexData(G,H,d,I,J);this.appendVertexData(K,L,d,M,N);this.appendVertexData(E,F,d,A,w);this.appendVertexData(G,H,d,I,J);this.appendVertexData(K,L,d,M,N);this.appendVertexData(O,a,d,P,Q);this.appendVertexData(G,H,d,I,J);this._dirty=!0}},drawPoly:function(a,c,d,e){if("object"===typeof a[0]){for(var g=b.length=0;g<a.length;g++)b.push(a[g].x,a[g].y);a=b}if(null==c)this._drawSegments(a,d,e,!0);else{null==c.a&&(c.a=255);null==e.a&&(e.a=255);d=null==d?this._lineWidth:d;d*=0.5;for(var k,
x,y,u,A,w,z,C=a.length,g=m.length=0;g<C;g+=2)k=a[(g-2+C)%C],x=a[(g-1+C)%C],y=a[g],u=a[g+1],A=a[(g+2)%C],w=a[(g+3)%C],f.x=x-u,f.y=y-k,h.x=u-w,h.y=A-y,cc.pNormalizeIn(f),cc.pNormalizeIn(h),u=f.x*h.x+f.y*h.y+1,z=(f.x+h.x)/u,u=(f.y+h.y)/u,m.push(z,u,h.x,h.y);C/=2;z=0<d;if(this._ensureCapacity(this._vertexCount+3*(3*C-2))){for(var B=!1==z?0.5:0,g=0;g<C-2;g++)k=a[0]-m[0]*B,x=a[1]-m[1]*B,y=a[2*g+2]-m[4*(g+1)]*B,u=a[2*g+3]-m[4*(g+1)+1]*B,A=a[2*g+4]-m[4*(g+2)]*B,w=a[2*g+5]-m[4*(g+2)+1]*B,this.appendVertexData(k,
x,c,0,0),this.appendVertexData(y,u,c,0,0),this.appendVertexData(A,w,c,0,0);var D,E,F,G;d=z?d:0.5;c=z?e:c;for(g=0;g<C;g++)e=(g+1)%C,k=a[2*g],x=a[2*g+1],y=a[2*e],u=a[2*e+1],f.x=m[4*g+2],f.y=m[4*g+3],h.x=z?-f.x:0,h.y=z?-f.y:0,D=m[4*g],E=m[4*g+1],F=m[4*e],G=m[4*e+1],e=k-D*d,A=x-E*d,w=y-F*d,B=u-G*d,k+=D*d,x+=E*d,y+=F*d,u+=G*d,this.appendVertexData(e,A,c,h.x,h.y),this.appendVertexData(w,B,c,h.x,h.y),this.appendVertexData(y,u,c,f.x,f.y),this.appendVertexData(e,A,c,h.x,h.y),this.appendVertexData(k,x,c,f.x,
f.y),this.appendVertexData(y,u,c,f.x,f.y);m.length=0;b.length=0;this._dirty=!0}}},_drawSegments:function(a,b,c,d){b=null==b?this._lineWidth:b;if(!(0>=b)){c=c||this._drawColor;null==c.a&&(c.a=255);b*=0.5;var e,g,k,y,u,A,w,z=a.length;for(w=m.length=0;w<z;w+=2)e=a[(w-2+z)%z],g=a[(w-1+z)%z],k=a[w],y=a[w+1],u=a[(w+2)%z],A=a[(w+3)%z],f.x=g-y,f.y=k-e,h.x=y-A,h.y=u-k,cc.pNormalizeIn(f),cc.pNormalizeIn(h),k=f.x*h.x+f.y*h.y+1,y=(f.x+h.x)/k,k=(f.y+h.y)/k,m.push(y,k,h.x,h.y);z/=2;if(this._ensureCapacity(this._vertexCount+
3*(3*z-2))){d=d?z:z-1;var C,B,D,E,F,G;for(w=0;w<d;w++)u=(w+1)%z,e=a[2*w],g=a[2*w+1],k=a[2*u],y=a[2*u+1],f.x=m[4*w+2],f.y=m[4*w+3],C=m[4*w],B=m[4*w+1],D=m[4*u],E=m[4*u+1],u=e-C*b,A=g-B*b,F=k-D*b,G=y-E*b,e+=C*b,g+=B*b,k+=D*b,y+=E*b,this.appendVertexData(u,A,c,-f.x,-f.y),this.appendVertexData(F,G,c,-f.x,-f.y),this.appendVertexData(k,y,c,f.x,f.y),this.appendVertexData(u,A,c,-f.x,-f.y),this.appendVertexData(e,g,c,f.x,f.y),this.appendVertexData(k,y,c,f.x,f.y);m.length=0;this._dirty=!0}}},clear:function(){this.release();
